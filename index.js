...
WAConnection,
        ...
        + 'FN:Ryuzki\n' // Seu nome
        + 'ORG:Bonin;\n' // Nome do Bot
        + 'TEL;type=CELL;type=VOICE;waid=5511941547847:+55 11941547847\n' //Seu número Whatsapp
        + 'END:VCARD'
        prefix = '.', '*', '#'
        blocked = []
        limitawal = '99999999999999999999999999999999999999999999999999999999999999999999'
        cr = '*Ryuzki*'
...
const client = new WAConnection()
        ...
        if (!welkom.includes(anu.jid)) return
                ...
                teks = `Flw, vai tarde já @${num.split('@')[0]} kkkkkkk 😂👋`
                        let buff = await getBuffer(ppimg)
                        client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
                }
                } catch (e) {
        ...
        if (!mek.hasNewMessage) return
                ...
                mess = {
                wait: 'Calma aí campeão, isso pode demorar',
                        success: '️❬ ✔ ❭ Pronto  🖤',
                        ...
                        levelnol: '*error* 0 °-°',
                        error: {
                        stick: '*Ops, falha, tente novamente ^_^*',
                                Iv: 'Desculpe, link inválido'
                        },
                        only: {
                        group: '[❗] Este comando só pode ser usado em grupos! ❌',
                                premium: '[❗] ESTE PEDIDO É SO PARA *USUÁRIOS PREMIUMS*',
                                mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO LINDO*',
                                benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
                                ownerG: '[❗] Bonin? Este é um recurso especial para o Bonin ❌',
                                ownerB: '[❗] Bonin? Este é um recurso especial para o Bonin ❌',
                                userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de BONIN. Registre-se para fazer amizade com o bot BONIN por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar BONIN|17\n\n──「 BONIN BOT 」──`,
        ...
        const botNumber = client.user.jid
                const ownerNumber = ["5511941547847@s.whatsapp.net"] // substitua isso pelo seu número/NÚMERO DO PROPRIETÁRIO
                const mod = [ownerNumber, "5511941547847@s.whatsapp.net"]//mude o seu numero/MOD DO BOT
                const adminbotnumber = ["5511941547847@s.whatsapp.net"]//mude o seu numero/ADM DO BOT
                const frendsowner = ["5511941547847@s.whatsapp.net"]//mude o seu numero/PROPRIETÁRIO DO BOT
                const premium = ["5511941547847@s.whatsapp.net", ]//PREMIUM adicionar premium Ale
                const premium = ["558688197913@s.whatsapp.net", ]//PREMIUM adicionar premium Yas
                const premium = ["5519993657837@s.whatsapp.net", ]//PREMIUM adicionar premium Farelo
                const premium = ["558197128865@s.whatsapp.net", ]//PREMIUM adicionar premium Sato
                const premium = ["555399676915@s.whatsapp.net", ]//PREMIUM adicionar premium Jaum
                const premium = ["557788028979@s.whatsapp.net", ]//PREMIUM adicionar premium Duda
                const premium = ["5511987540589@s.whatsapp.net", ]//PREMIUM adicionar premium Boccato
        ...
        if (messagesC.includes("pretooooo")){
        if (!isGroup) return
        ...
        if (messagesC.includes("seu macaco")){
        if (!isGroup) return
                if (!isAntiRacismo) return
                if (isGroupAdmins) return reply('cara, nao fale essas coisas, é errado, mas vc e admin n irei te banir')
                client.updatePresence(from, Presence.composing)
        ...
        if (!isGroup) return
                ...
                if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
                var kic = `${sender.split("@")[0]}@s.whatsapp.net`
                reply(`Sério cara? Tchau racista ${sender.split("@")[0]} você será expulso deste grupo em 5 segundos`)
                setTimeout(() => {
                client.groupRemove(from, [kic]).catch((e) => {reply(`*ERR:* ${e}`)})
                }, 5000)
                setTimeout(() => {
        ...
        if (messagesC.includes("pq vc é preto", "Seu preto")){
        if (!isGroup) return
                if (!isAntiRacismo) return
                if (isGroupAdmins) return reply('cara, não fale essas coisas, é errado, mas vc e admin n irei te banir')
                client.updatePresence(from, Presence.composing)
                if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
                var kic = `${sender.split("@")[0]}@s.whatsapp.net`
                reply(`Sério?? Tchau racista ${sender.split("@")[0]} você será expulso deste grupo em 5 segundos`)
                setTimeout(() => {
                client.groupRemove(from, [kic]).catch((e) => {reply(`*ERR:* ${e}`)})
                }, 5000)
                setTimeout(() => {
        ...
        if (!isGroup) return
                ...
                if (isGroupAdmins) return reply('vc é admin, então não irei te dar ban por usar links, rlx 🙂')
                client.updatePresence(from, Presence.composing)
                if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
                var kic = `${sender.split("@")[0]}@s.whatsapp.net`
                reply(`link detectado ${sender.split("@")[0]} você será expulso deste grupo em 5 segundos`)
                setTimeout(() => {
                client.groupRemove(from, [kic]).catch((e) => {reply(`*ERR:* ${e}`)})
                }, 5000)
                setTimeout(() => {
        ...
        client.updatePresence(from, Presence.composing)
                reply("vai vc, tu ja me disse q é mó bom")
        }
        ...
        media = await getBuffer(pp)
                ...
                gl2 = gh.split("|")[1];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/8bit/?text1=${gl1}&text2=${gl2}`, {method: 'get'})
                ...
                gl2 = gh.split("|")[1];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/bf4/?text1=${gl1}&text2=${gl2}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                case 'ceu':
                ...
                gl1 = gh.split("|")[0];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/shadowtext/?text=${gl1}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                case 'bugou':
                ...
                gl2 = gh.split("|")[1];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/tiktokeffect/?text1=${gl1}&text2=${gl2}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                case 'cafe':
                ...
                gl1 = gh.split("|")[0];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/coffeecup/?text=${gl1}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                case 'neon':
                ...
                gl1 = gh.split("|")[0];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/glowingneon/?text=${gl1}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                case 'desenholapis':
                ...
                gl1 = gh.split("|")[0];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/pencil/?urlgbr=${gl1}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                case 'caderno':
                ...
                gl1 = gh.split("|")[0];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/pencildrawing/?urlgbr=${gl1}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                case 'pubg':
                ...
                gl2 = gh.split("|")[1];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/pubgmlogo/?text1=${gl1}&text2=${gl2}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                case 'hpotter':
                ...
                gl1 = gh.split("|")[0];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/hpotter/?text=${gl1}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                case 'neon2':
                ...
                gl1 = gh.split("|")[0];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/darkneon/?text=${gl1}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                case 'gta':
                ...
                gl1 = gh.split("|")[0];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/gtavposter/?urlgbr=${gl1}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                ...
                gl1 = gh.split("|")[0];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/customwp/?urlgbr=${gl1}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                case 'phlogo':
...
//@Bonin					
        ...
        reply(teks.trim())
                ...
                gl3 = gh.split("|")[2];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/wanted/?urlgbr=${gl1}&text1=${gl2}&text2=${gl3}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                case 'naruto':
                ...
                gl1 = gh.split("|")[0];
                reply(mess.wait)
                anu = await fetchJson(`https://videfikri.com/api/textmaker/narutobanner/?text=${gl1}`, {method: 'get'})
                buff = await getBuffer(anu.result)
                client.sendMessage(from, buff, image, {quoted: mek})
                break
                break
        ...
        client.sendMessage(from, teks, text, {quoted: mek, detectLinks: false})
                ...
                client.sendMessage(from, `Pedidos recebidos, bloqueando  @${body.slice(7)}`, text)
                break
                case 'premiumlist':
                teks = '╭────*「 *PREMIUM USER👑* 」\n'
        ...
        client.sendMessage('556192247994@s.whatsapp.net', options, text, {quoted: mek})
                ...
                if (anu.error) return reply(anu.error)
                teks = `*Título* : ${anu.title}\n\n Espere 1 minuto, talvez um pouco mais porque o download de vídeos está executando`
                thumb = await getBuffer(anu.thumb)
                client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
                buffer = await getBuffer(anu.result)
                ...
                client.sendMessage(from, `Pedidos recebidos, desbloquear @${body.slice(9)}`, text)
                break
                case 'image':
        ...
        mentions(teks, members_id, true)
                ...
                hobby = body.slice(1)
                const hob = ['Jogar ps5', 'Dormir', 'Stalkear sua ex', 'Acho que não tenho nenhum agora', 'cozinhar', 'Ajudar', 'Nadar', 'Ler', 'Dormir', 'Te ajudar', 'Assistir anime', 'Assistir', 'Dirigir', 'Cantar', 'Dançar', 'Jogar', 'Falar sozinho']
        ...
        mentions(`Comando recebido, pedido de exclusão de mensagem : @${mentioned[0].split('@')[0]}`, mentioned, true)
                client.deleteMessage(from, mentioned)
        }
        break
                case 'openanime':
        ...
        break
                ...
                case 'totaluser':
                if (!isOwner) return reply(mess.only.ownerB)
                teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS BONIN BOT 👑* 」\n\`\`\``
                no = 0
...
// recursos anime, random @BONIN domina vida
        ...
        client.sendMessage(from, buf, image, {quoted: mek, caption: teks})
                break
                case 'owner':
                client.sendMessage(from, {displayname: "Alessander Vivo", vcard: vcard}, MessageType.contact, { quoted: mek})
        ...
        break
                case 'dado':
                if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o BONIN ou digite *.Daftarvip* para adquirir o acesso Premium!', text, { quoted: mek })
        ...
        break
                ...
                client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT BONIN ✨* 」\n*│+ wa.me/5511941547847*╰──────*「 *BONIN* 」*────*\n\n*_SE QUER SER ADMIN DO BOT BONIN_*\n*_Tipo /iklan_*' })
                break
                case 'iklan':
        ...
        teks += `Total : ${mod.length}`
                ...
                case 'bot':
                memein = await kagApi.memeindo()
                buffer = await getBuffer(`https://i.pinimg.com/564x/b6/b3/13/b6b313a1aacda286595cd7198ea7570e.jpg`)
                ...
                client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hmm, então você quer ver loli?'})
                break
                case 'hentai':
                memein = await kagApi.memeindo()
                buffer = await getBuffer(`https://i.imgur.com/8U9GwX4.jpg`)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Cara bate pra 2d kkkkkkk😂'})
                break
                case 'bomdia':
                memein = await kagApi.memeindo()
                buffer = await getBuffer(`https://i.imgur.com/7VL9cFf.jpg`)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia, vcs são fodas ❤️'})
                break
                case 'boatarde':
                memein = await kagApi.memeindo()
                buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa tarde rapaziada'})
                break
                case 'boanoite':
                memein = await kagApi.memeindo()
                buffer = await getBuffer(`https://i.imgur.com/ctFYJUe.jpeg`)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa noite seus lindos'})
                break
                case 'sexta':
                memein = await kagApi.memeindo()
                buffer = await getBuffer(`https://seriesedesenhos.com/images/2019/08/12/batman3.gif`)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Hoje é sextaaaa dia de ficar mais louco que o Batman'})
                break
                case 'belle':
                ...
                client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do BONIN:*\n\n https://youtube.com/channel/UCpB3qh2Sp3K23s9a2Q-Gf-g'})
                break
                case 'nsfwloli1':
                memein = await kagApi.memeindo()
                ...
                case 'memeindo':
                memein = await kagApi.memeindo()
                buffer = await getBuffer(`https://i.pinimg.com/236x/77/2f/f5/772ff504fcbaac7b0d0928353e010375.jpg`)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
                break
                case 'dono':
                memein = await kagApi.memeindo()
                buffer = await getBuffer(`https://pbs.twimg.com/profile_images/1364230675012853766/oFpHQnyI_400x400.jpg`)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Ryuzki\n*WPP:* wa.me/+5511941547847\n*INSTA:* @bot_bonin\n\n\nEspero que tenham gostado do BOT'})
                break
                case 'setprefix':
                if (args.length < 1) return
                if (!isOwner) return reply(mess.only.ownerB)
                ...
                break
                case 'nsfwloli':
                if (!isNsfw) return reply('*COMANDO SÓ FUNCIONA NO PV AMIGO E É +18.*')
                loli.getNSFWLoli(async (err, res) => {
                if (err) return reply('❌ *ERRO* ❌')
                        buffer = await getBuffer(res.url)
                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'SE TRATE AMIGO, SÉRIO...'})
                })
                break
                case 'pronomeneu':
